...
ParticleSystem.prototype.update = function()
{
	...
	if(dt>1) dt = 1/60;
	var m = [
		this.oldC[0]-this.c[0],
		this.oldC[1]-this.c[1],
		this.oldC[2]-this.c[2]];
	this.oldC = this.c.slice();
	if( dt> 0)
	{
		var toDel = [];
	...
	for(var i=0;i<nNew;i++)
	{
		...
		p[1] = sa * r;      // posY
		p[2] = ca * cb * r; // posZ
		p[3] = -p[0]*(Math.random()*0.2+0.9); // velX
		p[4] = Math.random()-0.5; // velY
		p[5] = -p[2]*(Math.random()*0.2+0.9); // velZ
	...
	{
		var p = this.particles[i];
		this.array[j+0] = p[0]+this.c[0];
		this.array[j+1] = p[1]+this.c[1];
		this.array[j+2] = p[2]+this.c[2];
		this.array[j+3] = p[6];
	}
	this.mean = [0,0,0,0];
	var n = 0;
	...
	for(var i=0;i<this.particles.length;i++)
	{
		...
		if(p[6]<=1)
		{
		var d = [(this.mean[0]-p[0]),(this.mean[1]-p[1]),(this.mean[2]-p[2])];
